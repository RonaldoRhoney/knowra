...
export function Perfil() {
  ...
  async function handleAvatarChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/")) {
      setErroAvatar(t("perfil.escolhaImagem"));
      return;
    }
    if (file.size > TAMANHO_MAX_MB * 1024 * 1024) {
      setErroAvatar(t("perfil.imagemMuitoGrande", { mb: TAMANHO_MAX_MB }));
      return;
    }
    ...
    if (erroUpload) {
      setErroAvatar(t("perfil.erroEnviarFoto"));
    ...
    if (erroUpdate) {
      setErroAvatar(t("perfil.fotoEnviadaSemAtualizar"));
    } else {
  ...
  async function handleSalvarRanking() {
    ...
    if (nickname.trim().length > 0 && nickname.trim().length < 2) {
      setErroRanking(t("perfil.apelidoCurto"));
      return;
    }
    ...
    const { error } = await supabase
      ...
      .eq("id", session.user.id);
    if (error) {
      setErroRanking(t("perfil.erroSalvarGenerico"));
    } else {
      await refreshProfile();
  ...
  async function handleAssinar() {
    ...
    try {
      ...
      setErroAssinatura(t("perfil.erroAssinatura"));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (error) {
      setErro(t("perfil.erroSalvarGenerico"));
    } else {
      await refreshProfile();
      setSalvo(true);
    }
  ...
  return (
    ...
    <div className="px-4 py-8 max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-h2">{t("nav.perfil")}</h1>
        <p className="text-sm text-knowra-text-secondary mt-0.5">{t("perfil.gerencie")}</p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-2">
        <button
          ...
          title={t("perfil.trocarFotoPerfil")}
        >
          {profile.avatar_url ? (
          ...
          <div className="absolute inset-0 rounded-full bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity grid place-items-center text-[10px]">
            {enviandoAvatar ? "..." : t("perfil.trocar")}
          </div>
        </button>
        <input
          ref={inputAvatarRef}
        ...
        />
        <div>
          <p className="text-sm font-medium">{profile.nome ?? t("perfil.semNome")}</p>
      ...
      <div className="bg-knowra-surface rounded-2xl p-5 mb-4">
        {assinatura?.plano === "pro" ? (
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-knowra-text-secondary">{t("perfil.assinaturaAtiva")}</p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-1">
              <p className="text-sm font-medium">KnowRa Pro</p>
              <span className="text-xs text-knowra-text/40">R$ 19,90/{t("perfil.mes")}</span>
            </div>
            <p className="text-xs text-knowra-text-secondary mb-4">{t("perfil.beneficiosPro")}</p>
            {erroAssinatura && <p className="text-sm text-red-400 mb-3">{erroAssinatura}</p>}
            {assinatura?.status_assinatura === "pending" && (
              <p className="text-xs text-knowra-text/50 mb-3">{t("perfil.assinaturaProcessando")}</p>
            )}
            <button
              type="button"
            ...
            >
              {iniciandoCheckout ? t("perfil.abrindoCheckout") : t("perfil.assinarPro")}
            </button>
          </>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-knowra-surface rounded-2xl p-5 space-y-3">
        <div>
          <label className="text-xs text-knowra-text/50 mb-1 block">{t("cadastro.nome")}</label>
          <input
            value={nome}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-knowra-text/50 mb-1 block">{t("perfil.cidade")}</label>
            <input
              value={cidade}
          ...
          <div>
            <label className="text-xs text-knowra-text/50 mb-1 block">{t("perfil.pais")}</label>
            <input
              value={pais}
        ...
        <div>
          <label className="text-xs text-knowra-text/50 mb-1 block">{t("perfil.idade")}</label>
          <input
            value={idade}
        ...
        <div>
          <label className="text-xs text-knowra-text/50 mb-1.5 block">{t("perfil.genero")}</label>
        ...
        {salvo && <p className="text-sm text-emerald-400">{t("perfil.perfilAtualizado")}</p>}
        ...
        >
          {enviando ? t("cadastro.salvando") : t("perfil.salvarAlteracoes")}
        </button>
      </form>

      <div className="bg-knowra-surface rounded-2xl p-5 mt-4">
        <p className="text-xs text-knowra-text/50 mb-3">{t("perfil.progresso")}</p>
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p className="text-knowra-text/40 text-xs">{t("progresso.nivel")}</p>
            <p className="font-medium">{profile.nivel_global}</p>
          </div>
          <div>
            <p className="text-knowra-text/40 text-xs">{t("perfil.xpTotal")}</p>
            <p className="text-knowra-accent font-medium">{profile.xp_total}</p>
          </div>
          <div>
            <p className="text-knowra-text/40 text-xs">{t("perfil.perguntasFeitas")}</p>
            <p className="font-medium">{resumo?.total_perguntas ?? "—"}</p>
          </div>
          <div>
            <p className="text-knowra-text/40 text-xs">{t("perfil.taxaAcerto")}</p>
        ...
        {areasDestaque.length > 0 && (
          <div className="mt-4 pt-4 border-t border-white/10">
            <p className="text-knowra-text/40 text-xs mb-2">{t("perfil.areasDestaque")}</p>
            <div className="flex flex-wrap gap-1.5">
        ...
        <Link to="/mapa" className="text-xs text-knowra-accent hover:underline mt-4 inline-block">
          {t("perfil.verMapaCompleto")} →
        </Link>
      </div>

      <div className="bg-knowra-surface rounded-2xl p-5 mt-4">
        <div className="flex items-center justify-between mb-1">
          <p className="text-sm font-medium">{t("nav.ranking")}</p>
          <Link to="/ranking" className="text-xs text-knowra-accent hover:underline">
            {t("perfil.verRanking")} →
          </Link>
        </div>
        <p className="text-xs text-knowra-text/50 mb-4">{t("perfil.explicacaoRanking")}</p>

        <div>
          <label className="text-xs text-knowra-text/50 mb-1 block">{t("perfil.apelidoRanking")}</label>
          <input
            value={nickname}
            onChange={(e) => setNickname(e.target.value)}
            placeholder={t("perfil.placeholderApelido")}
            maxLength={24}
            className="w-full rounded-lg bg-knowra-bg border border-white/10 px-3 py-2 text-sm outline-none focus:border-knowra-primary"
          />
        </div>
        ...
        >
          <span className="text-sm">{t("perfil.aparecerRankingPublico")}</span>
          <span
        ...
        {salvoRanking && <p className="text-sm text-emerald-400 mt-3">{t("perfil.preferenciasSalvas")}</p>}
        ...
        >
          {salvandoRanking ? t("cadastro.salvando") : t("perfil.salvarPreferencias")}
        </button>
      </div>
    </div>
    <Footer />
